	...
	fun loadCacheList(type: Int, init: () -> Unit, observer: Observer<List<InstallAPP>>) {
		Observable.create<List<InstallAPP>> { subscriber ->
			...
			val fileName: String = when (type) {
				AppManagerUtil.AppType.USER -> "${JanYoFileUtil.USER_LIST_FILE}${Settings.sortType}"
				AppManagerUtil.AppType.SYSTEM -> "${JanYoFileUtil.SYSTEM_LIST_FILE}${Settings.sortType}"
				else -> {
